...
- **Tunnels** - expose a service running in the sandbox at a public HTTPS URL: `POST /api/tunnels` body `{ "workspace_id", "command", "port", "name"?, "timeout"?, "bindings"? }` → `{ url, tunnel_id, … }`. Max 3 active per workspace; timeout defaults 5 min, caps at 30. `GET /api/tunnels?workspace_id=…`, `DELETE /api/tunnels/{id}`. `bindings` is an explicit launch-time array such as `[{ "alias": "github", "provider": "github", "connectionId": "…" }]`. Portable declarations such as `{ "integrations": { "github": { "provider": "github" } } }` stay in the `.server` file and are not sent in the tunnel request. Do not store a connection id or secret in the file.
