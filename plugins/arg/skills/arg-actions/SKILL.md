...
- **video / 3d / audio** — `video_generate` (text/image/video→video; also drives audio-driven avatar / lip-sync models like Kling AI Avatar - pass the portrait as `source_path` and the voice track as `audio_path`), `three_d_generate`, `video_to_audio`, `enhance_audio` (clean up recorded speech: `fal-ai/deepfilternet3` removes background noise from an audio file, `fal-ai/elevenlabs/audio-isolation` isolates the voice from music/noise/room echo and also accepts a video; output is the same length as the input), `music_generate`, `tts_generate` (general text→speech: defaults to ElevenLabs MP3; set `model_id` to `gemini-3.8-flash-tts` or `gemini-3.8-flash-lite-tts` for Gemini WAV), `gemini_flash_tts_generate` (expressive Gemini 3.8 Flash speech, WAV), `gemini_flash_lite_tts_generate` (fast Gemini 3.8 Flash-Lite speech, WAV), `sound_effect_generate` (text→sound effect), `transcribe_audio`. For Gemini, pass verbatim `text`, optional `voice` (defaults to Kore) and optional `style`; do not pass an ElevenLabs voice id or output format. Use a `.wav` output path.
...
- **integration** — calls to connected services where a connection is set up. Google products use separate connections; Google Meet is coming soon. Each action takes a `connection` input - use `describe_action(action_id, "connection")` to list the ones available. One provider per line, alphabetical - add a new one on its own line:
  ...
  - Monday.com
  - Notion
  - Parallel (objective-driven web search and extraction, plus async research/enrichment task runs)
