...
- **integration** — calls to connected services where a connection is set up: Airtable, Ashby (recruiting - read the hiring pipeline, search candidates, move an application to another interview stage, note a candidate), Confluence, Databricks (run SQL on a warehouse, browse Unity Catalog, run and check jobs), Discord (bot token - send a channel message, list recent messages), Exa (web search plus clean page extraction, and async agent runs for deep research), Firecrawl (scrape a page, map a site, search with the hits scraped, crawl a domain), Firmable (ANZ company/people data), GitHub, Gong (what was said on sales calls - briefs, key points, trackers, transcripts), Google Calendar, Google Drive (including `google_append_sheet_row` and `google_read_sheet_range` for spreadsheets), Google Keep, HubSpot, Jira, Linear, Microsoft, Monday.com, Notion, Parallel (objective-driven web search and extraction, plus async research/enrichment task runs), Resend (send one email or a batch, check what happened to it, manage contacts and broadcasts), Salesforce, SendGrid (send transactional email, manage marketing contacts, read delivery statistics), Slack, Smartlead (cold outreach - campaign analytics, leads, lead conversations, and replying on the thread), Stripe (payments - get a charge, create a refund), treg (a gateway to thousands of third-party API endpoints - search its catalogue, then call an endpoint by id), Twilio, Vapi, and Zendesk (support tickets - create a ticket, add a reply/internal note, change status/priority/tags). Google products use separate connections; Gmail and Google Meet are coming soon. Each action takes a `connection` input - use `describe_action(action_id, "connection")` to list the ones available.
