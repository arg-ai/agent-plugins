...
- **integration** — calls to connected services where a connector is set up: Airtable, Confluence, Firmable (ANZ company/people data), GitHub, Google (Gmail/Calendar/Drive), HubSpot, Jira, Linear, Microsoft, Monday.com, Notion, Salesforce, Slack, treg (a gateway to thousands of third-party API endpoints - search its catalogue, then call an endpoint by id). Each takes a `connection` input - use `describe_action(action_id, "connection")` to list the ones available.
