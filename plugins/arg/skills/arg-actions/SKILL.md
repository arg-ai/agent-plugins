...
- **integration** — calls to connected services where a connector is set up: Airtable, Confluence, Discord (bot token - send a channel message, list recent messages), Firmable (ANZ company/people data), GitHub, Google (Gmail/Calendar/Drive/Sheets - `google_append_sheet_row`/`google_read_sheet_range` read and write a spreadsheet), HubSpot, Jira, Linear, Microsoft, Monday.com, Notion, Salesforce, Slack, Stripe (payments - get a charge, create a refund), treg (a gateway to thousands of third-party API endpoints - search its catalogue, then call an endpoint by id), Databricks (run SQL on a warehouse, browse Unity Catalog, run and check jobs), Gong (what was said on sales calls - briefs, key points, trackers, transcripts), Exa (web search plus clean page extraction, and async agent runs for deep research), Firecrawl (scrape a page, map a site, search with the hits scraped, crawl a domain), Parallel (objective-driven web search and extraction, plus async research/enrichment task runs), SendGrid (send transactional email, manage marketing contacts, read delivery statistics), Resend (send one email or a batch, check what happened to it, manage contacts and broadcasts), Smartlead (cold outreach - campaign analytics, leads, lead conversations, and replying on the thread), Ashby (recruiting - read the hiring pipeline, search candidates, move an application to another interview stage, note a candidate), Zendesk (support tickets - create a ticket, add a reply/internal note, change status/priority/tags). Each takes a `connection` input - use `describe_action(action_id, "connection")` to list the ones available.
