...
- **web / data / document** — `screenshot_webpage`, `web_to_markdown` (one-shot page → Markdown), `web_fetch` (the full browser surface: `format` of `markdown` / `html` / `links` / `scrape` / `pdf` / `crawl`), `extract_webpage_data`, `web_scrape` (curated Apify scrapers — Instagram, LinkedIn, X, YouTube, G2, Trustpilot, …), `get_stock_data`, `html_to_pdf`, `ad_search` / `advertiser_search` / `ad_creatives` (the public Meta, TikTok and Google ad libraries - what ads a brand is running, and saving an ad's image/video creatives into the workspace; public ad transparency data, not your own ad accounts).
- **integration** — calls to connected services where a connection is set up: Airtable, Ashby (recruiting - read the hiring pipeline, search candidates, move an application to another interview stage, note a candidate), Confluence, Databricks (run SQL on a warehouse, browse Unity Catalog, run and check jobs), Discord (bot token - send a channel message, list recent messages), Exa (web search plus clean page extraction, and async agent runs for deep research), Firecrawl (scrape a page, map a site, search with the hits scraped, crawl a domain), Firmable (ANZ company/people data), GitHub, Gmail (messages, threads, and Google Contacts), Gong (what was said on sales calls - briefs, key points, trackers, transcripts), Google Calendar, Google Drive (the complete Docs, Sheets, Slides, and Drive REST catalogs, plus convenience actions such as `google_append_sheet_row`), Google Keep, HubSpot, Jira, Linear, Microsoft, Microsoft Teams (chat messages/search and Outlook Contacts), Monday.com, Notion, Parallel (objective-driven web search and extraction, plus async research/enrichment task runs), Resend (send one email or a batch, check what happened to it, manage contacts and broadcasts), Salesforce, SendGrid (send transactional email, manage marketing contacts, read delivery statistics), Slack (channels, DMs, messages, and workspace members), Smartlead (cold outreach - campaign analytics, leads, lead conversations, and replying on the thread), Stripe (payments - get a charge, create a refund), treg (a gateway to thousands of third-party API endpoints - search its catalogue, then call an endpoint by id), Twilio, Vapi, and Zendesk (support tickets - create a ticket, add a reply/internal note, change status/priority/tags). Google products use separate connections; Google Meet is coming soon. Each action takes a `connection` input - use `describe_action(action_id, "connection")` to list the ones available.
