...
- **Bookmarks / shortcuts** — `.url` (Windows INI: `[InternetShortcut]` with a `URL=` line), `.webloc` (macOS plist with a `URL` key — XML when hand-authored, but macOS Finder commonly writes the binary `bplist00` form instead, so don't assume a `.webloc` you read raw is XML text). Clickable files that open a single URL in a new tab; name them after the host (e.g. `github.com.url`).
- **App launchers** - `.app`. A small version 2 JSON manifest that gives an app a name, inline SVG icon and launchable target of its own. A file launcher uses `{"version":2,"name":"Expense tracker","icon":"<svg viewBox=\"0 0 24 24\">...</svg>","entry":{"kind":"file","id":"argfile_<uuid>","path":"/apps/expenses.tsx"}}`; `path` names an existing `.html`/`.htm`/`.tsx`/`.jsx`/`.server` file (legacy `.site` server files also work) and the paired id lets it follow a rename. A hosted Site launcher uses `{"version":2,"name":"Portfolio","icon":"<svg viewBox=\"0 0 24 24\">...</svg>","entry":{"kind":"site","site_id":"<site-id>","id":"argfile_<folder-uuid>","path":"/sites/portfolio"}}`; `site_id` selects the deployed Site while `id` and `path` refer to its source folder. Opening the `.app` renders the authored app, launches the server, or opens the Site's current live version on web, desktop, iOS and Android. An optional top-level `permissions` object (`{"files":{"access":"readwrite","scope":"folder"},"actions":["file_read","slack_*"],"devices":["camera"]}`) declares what the app needs, and the user is asked once for all of it on open; an `actions` entry may be an exact id, a `prefix_*` wildcard or `"*"` for every Action - see `arg-apps`. The launcher is a pointer, not a copy - edit the file, server, or Site source rather than putting source inside the manifest.
- **3D / CAD / splats** — `.glb`, `.gltf`, `.obj`, `.stl`, `.fbx`, `.usd*`, `.ply`, `.dae`, `.3ds`, `.3mf`, `.vox`, `.vrml`/`.wrl`, `.amf`, `.blend` (Blender scenes — base meshes, materials and object hierarchy; modifiers are reported, not applied); CAD `.step`/`.stp`, `.iges`/`.igs`, `.brep`, `.ifc`, `.3dm`, `.dxf`, `.dwg`; PCB layouts `.kicad_pcb` (KiCad — upload/view only); splats `.splat`, `.ksplat`, `.spz`.
