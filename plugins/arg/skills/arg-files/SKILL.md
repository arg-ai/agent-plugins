...
**Already installed — import, don't install:** `requests`, `pandas`, `numpy`, `scipy`, `matplotlib`, `pillow`, `pillow-heif`, `beautifulsoup4`, `pyyaml`, `openpyxl`, `xlsxwriter`, `python-docx`, `python-pptx`, `pymupdf`, `pypdf`, `pypdfium2`, `pdfplumber`, `opencv-python-headless`, `imageio-ffmpeg`. `ffmpeg` and `ffprobe` are on PATH — do not `apt-get install ffmpeg` or `pip install imageio-ffmpeg`.
