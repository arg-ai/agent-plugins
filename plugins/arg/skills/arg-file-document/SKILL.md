...
- **`<Recording />`** — block void. A persistable meeting-recording card (Notes / Transcript / Summary tabs).
  - Identity: `recordingId` — the block's durable handle, written by whichever client captured it. The server fills a finished recording in by finding the block with this id, so leave it alone: changing or removing it cuts the block loose from its own recording.
...
| Component                                                                                      | Props                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<FormInput type="text" name="email" label="Email" required />`                                | `name` (required — the CSV column key, lower_snake_case), `label` (question text), `type` (text \| email \| number \| tel \| url \| date \| time \| checkbox \| country \| yesno \| color \| file; default text — `country` is a searchable country dropdown, `yesno` renders fixed Yes/No buttons, `color` renders a color swatch picker, and `file` uploads one file up to 25 MB into the workspace), `placeholder` (text-like inputs only), `required` (shorthand). `type="number"` also takes `min`/`max`/`step`; `type="date"` takes `dateRange="future"` or `"past"`; `type="file"` takes `accept` (an HTML file-picker filter such as `".pdf,image/*"`).                                                                          |
