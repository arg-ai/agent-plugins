...
For how to use the UI, permissions, sharing, settings or platform-specific features, read the relevant product guide before answering:

- Discover it through https://arg.ai/docs/llms.txt and fetch the page URL with `.md` appended, for example https://arg.ai/docs/permissions.md. Each guide's `/llms.txt` gives an outline and links to sub-pages.
- Read the platform-specific guide when steps differ between web, desktop and mobile. Answer from the guide and link the user to the normal page URL.
- If a guide cannot be fetched or does not cover the question, say what is unverified instead of inventing controls or permission rules.

Keep detailed UI instructions in those guides. API contracts live at https://developers.arg.ai; available tools and their schemas determine what the agent can execute. For file operations, use `arg-files` and the access-method and format skills it names.
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **Files & filesystem**         | Drive-style manager (upload, folders, previews, sharing, bookmarks) on one filesystem served to every surface — web, local mount, sandbox, REST/CLI/MCP                                                                                                                                                                   | `arg-files` (file CRUD), [Workspaces](https://arg.ai/docs/workspaces.md) |
...
| **The agent**                  | Every workspace ships with an AI agent: chats over your files with cited sources, edits inline in every editor, runs Python/Bash in an isolated sandbox, generates media, works long tasks in the background, runs on your choice of frontier model                                                                       | [Chat](https://arg.ai/docs/chat.md)                                      |
...
| **Arg Apps**                   | A single `.html` file becomes an internal app (dashboard, CRM, tracker) via `@arg-ai/sdk` - workspace files are its backend. `.server` runs a sandboxed process with Personal, Workspace, or Public access                                                                                                                | `arg-apps`                                                               |
...
| **Recordings & meeting notes** | Record mic/screen (desktop detects meetings), transcribe, and summarize into structured notes with decisions and action items                                                                                                                                                                                             | [Record](https://arg.ai/docs/voice.md)                                   |
...
| **Collaboration & activity**   | Live co-editing, per-file comments and threads, version history, and a filterable audit trail covering UI, API, MCP, agents, and automations                                                                                                                                                                              | [Permissions](https://arg.ai/docs/permissions.md)                        |
| **Connectors (BYO MCP)**       | Plug external MCP servers and integrations into a workspace so the agent can act in other systems (Settings → Connectors)                                                                                                                                                                                                 | [Integrations](https://arg.ai/docs/integrations.md)                      |
...
| **Desktop** (macOS/Windows/Linux)         | Local-folder workspaces, folder sync, offline access, drag to/from Finder/Explorer, native notifications, meeting detection, local Claude Code/Codex agents, a local MCP server exposing shared folders to external agents | [Desktop](https://arg.ai/docs/desktop.md)    |
| **Mobile** (iOS/Android)                  | Chat with file context on the go, snap photos into a workspace, voice prompts, push notifications                                                                                                                          | [Mobile](https://arg.ai/docs/mobile.md)      |
...
- **The `@arg-ai/sdk` package** turns a previewed `.html` file into a file-backed app — see `arg-sdk` and https://developers.arg.ai/guides/sdk/overview.

## Go deeper: which guide or skill to read next

| The question or task is about…                                               | Read                                                              |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------- |
...
| UI, permissions, sharing, settings, choosing a surface                       | [Product docs](https://arg.ai/docs/llms.txt)                      |
