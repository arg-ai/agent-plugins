...
| **Editors & native formats**   | Purpose-built editors for every file type: notes (`.md`/`.mdx`), whiteboards, kanban boards, native slides (`.design`) and PowerPoint (`.pptx`), spreadsheets (`.csv`), PDF, design (`.design`/`.svg`), video NLE (`.video`), DAW (`.daw`), forms, diagrams, 3D/CAD, code, and many more | `arg-files` → `arg-file-*`; `arg-slides` for decks |
