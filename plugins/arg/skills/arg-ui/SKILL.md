...
- **Integration imports** - behind the default-off PostHog flag `allow_workspace_importing`, the file browser toolbar's More menu has an **Import** row that opens a full-page importer as a workspace tab. Notion is available now: paste links to the pages and databases you want (several at once) and each is looked up and listed before you import. Each entry has its own `Sub-pages` checkbox, on by default, deciding whether what is nested inside it comes too. Everything lands at the Arg workspace root: pages become `.mdx`, databases become `.csv` with a matching Pages/Kanban view, and nested content keeps its folder structure. A database's rows are always in its `.csv`; its checkbox only decides whether each row also becomes its own page. An import runs on the server, so closing the tab does not stop it - progress, cancellation and the finished result are reported by a floating indicator, and it survives a reload. The `+ New` menu and mobile still open the same importer as a modal, where Google Drive, Dropbox, Microsoft and Jira remain coming soon.
