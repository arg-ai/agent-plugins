...
- **Integration triggers** (`category: "trigger"`, `kind: "integration-trigger"`): starts the flow from a connected provider's own events instead of polling/webhooking it yourself - `config: {providerId, triggerName, connectionId, ...filterFields}`, e.g. `{providerId: "google", triggerName: "gmail_message", connectionId: "<existing connection id>", query: "in:inbox is:unread"}` fires on new Gmail mail matching that search (`query` is optional - omit it to fire on every new message). `connectionId` must already exist (connect the provider first in Integrations settings); `filterFields` vary per trigger, and **the full catalogue is `llms.txt` beside this file** - every provider, every `triggerName`, its filter fields with types, and the scopes the connection must hold, generated from the registry the workflow editor's palette renders. Read it before choosing a trigger rather than recalling one: it is the difference between a `triggerName` that exists and one that compiles and never fires. Fetch it at https://api.arg.ai/skills/content/arg-file-automation/llms.txt when the file isn't on disk beside this one. Event data lands at `{{ trigger.output.data.… }}` (e.g. `{{ trigger.output.data.subject }}` for `gmail_message`). In YAML this is `on.integration: {provider, trigger, connection, ...filterFields}` (same three binding fields, spelled without the `Id`/`Name` suffixes since they're already under `on:`) - see the YAML trigger table below.
- **Actions** (`category: "action"`): `code`, `http-request`, `read-file`, `write-file`, `append-file`, `list-files`, `grep`, `edit-file` (`{path, edits: [{operator, value, replacement, replace_all}]}`), `copy-file` / `move-file` (`{sourcePath, destinationPath, onConflict}`; `onConflict` is `"append"` (default - keep both, suffixing `(1)`, `(2)`, … on collision) or `"overwrite"` - replace the existing destination file), `delete-file`, `run-llm`, `screenshot`, `fetch-web`, `download-file` (raw GET of a URL saved to a workspace file, keeping the source's content type - `{url, savePath?, contentType?, headers?}`; good for RSS feeds), `extract-data`, `apify-actor` (curated Apify web scrapers; `actorKey` picks the scraper, `input` is that actor's input object; returns `{items, itemCount, actorId}`), `run-automation` (`{filePath, triggerInput?, waitForCompletion?, waitTimeoutSeconds?}`: starts another `.automation` or `.arg/workflows/*.yml` file in the same workspace as a sub-run - the composition primitive for splitting a big pipeline into smaller automations, or fanning out a `loop` body over one sub-automation per item. `filePath` must be a `.automation` or `.arg/workflows/*.yml` path. Fire-and-forget by default; set `waitForCompletion: true` to block on the sub-run and read `{{ <id>.output.node_results }}` - a failed or timed-out sub-run reports `{{ <id>.output.status }}` as `"error"` / `"timeout"` rather than failing this node, so branch on it with `if` instead of assuming success. A sub-run that would call back into its own caller, directly or through a longer chain, is refused before it starts), `send-notification` (`{title, body, notifyRunner, recipientUserIds}`: delivers a workspace notification. `notifyRunner` defaults `true` and notifies whoever runs the automation - the running user on manual runs, the workspace owner on scheduled runs. `recipientUserIds` is an optional array of workspace-member user ids to also notify; ids outside the workspace's org are ignored).
...
- **Flow control** (`category: "flow"`): `if` (has a `false` output handle for the else branch), `switch` (one handle per case), `loop`, `merge` (`mode`: `wait-all` / `first`), `delay` (`{mode?, amount, unit}` or `{mode: "until", until}`; `mode: "relative"` is the default — `unit` is `seconds` / `minutes` / `hours` / `days`, capped at 7 days total; `mode: "until"` waits until an absolute ISO-8601 date/time in `until` instead — almost always a `{{ Node.output.path }}` template pulling a date out of upstream data, since the expression language has no date arithmetic to compute a relative offset by hand, and a moment already in the past continues immediately rather than erroring. Either mode pauses this branch of a deployed run without using any compute; a manual live-preview run in the editor caps to a short simulated wait instead, since it has no durable step to sleep on), `filter` (`{arrayPath?, expression?}`; narrows an array to the items matching a per-item condition before it reaches a `loop` or the rest of the branch — `arrayPath` picks the array to filter (`{{ Node.output.path }}`, a plain dot-path like `body.items`, or blank to filter the node's own input), `expression` is evaluated once per item the same way `if`'s expression mode is (`status == 'open'`; blank keeps every truthy item). Output is `{items, keptCount, droppedCount, totalCount, truncated}` - capped at 5000 items considered).
...
- `name` — display label.
- `uses` — an action kind (`read-file`, `write-file`, `copy-file`, `move-file`, `run-llm`, `run-automation` (see below), `http-request`, `send-notification`, …), `action/<action_id>` to run any action from the central registry (`with:` is then that action's own input, e.g. `uses: action/image_generate`), or a flow kind (`switch`, `merge`, `delay` (`with: {amount, unit}` or `with: {mode: until, until}` — see Flow control above), `filter` (`with: {arrayPath?, expression?}` — see Flow control above), or `loop` — see Loops below). **`uses: if` is a compile error** — conditionals are the step-level `if:` key below.
...
- `deploy_automation` returns `missing_access` when the service account can't reach a file one of the document's nodes names — each entry giving the path, the level and the node that will fail on it. Hand those exact paths to `request_service_account` and deploy again; every entry is a trigger that fails on its next fire.
- A permission failure has one fix, and it isn't editing the graph. `missing_access` on a deploy, or a `failed_on_missing_access` verdict from a run, means the account's scope is short: call `request_service_account` again with the paths it named and `current_service_account_id` set to the account the automation already uses — the card then asks the user to widen that account instead of picking one again — then deploy and run it again.
- `run_automation({ file_path, trigger_input?, wait_seconds? })` — run it now and wait for the verdict, so you find out whether it works in this turn instead of at 6am. Deploy first, then run: a **deployed** automation runs as its service account, the same principal its triggers use, so a missing grant fails here exactly as dispatch would, while an undeployed file runs as you and proves only that the graph is sound. Pass `trigger_input` to stand in for what the real trigger would deliver (the webhook body, the changed file). A deployed automation runs its **snapshot**, so after editing the file deploy again before running - the result's `ran_document` and `file_changed_since_deploy` tell you when a verdict is about the old graph. The run is real — it sends the emails and writes the files — so don't run one whose side effects the user hasn't agreed to, and a `still_running` result is not a verdict.
...
Deploy validates the configured service account against the automation file and every file the workflow reads or writes. Persistent triggers run **as that service account**, not as you or the user, so `service_account_id` (`service-account:` in YAML) must be the id (a UUID) of an **existing** service account with access to those files — **never invent or guess an id**. Don't know which to use? Deploy anyway: the error lists the organization's service accounts by id and name. When exactly one fits, set it and deploy again without asking. When the choice isn't obvious — none exist yet, or several could apply — call `request_service_account({ purpose, automation_file_path, access })` (chat surface): it pauses on an interactive picker where the user selects an existing account or creates one inline, and returns the chosen id to write into the file. **Pass `automation_file_path` whenever the file exists**: its nodes are scanned for the files they read and write, and those paths go into the grant, so the scope matches the workflow rather than your reading of it — the single commonest cause of "this automation doesn't have access to this file" days later. `access.level` (`read` / `write` / `manage`) still applies, raised automatically when the document needs more; `access.paths` is for what a scan can't see (the files a `code` node or a subagent will touch), and the result's `access_not_derivable` names exactly those nodes. `access.whole_workspace: true` remains for an automation that genuinely operates workspace-wide. Paths that don't exist yet are fine; the grant applies once the automation creates them. The picker shows that list to the user before they choose, and the grants are written under **their** permissions the moment they pick — so check the result's `access_not_granted` and relay anything that was refused, because the automation will fail on those paths at run time. If the picker is dismissed or unavailable (CLI/MCP), ask the user instead. An automation whose trigger nodes are all disabled deploys as `paused`.
...
- Prefer `run-automation` over one giant graph when a workflow has a clear "for each item, do a multi-step thing" shape or a "stage two only needs to know stage one finished" shape - it keeps each piece independently testable and deployable, and `waitForCompletion` plus `{{ <id>.output.status }}` gives the parent graph real control over the fan-out instead of guessing.
- A deploy is not a test. Deploy, then `run_automation` once and read the verdict — the failure it catches (the service account can't reach a file the workflow touches) is invisible to every other check and only shows up as a silent 6am no-op.
