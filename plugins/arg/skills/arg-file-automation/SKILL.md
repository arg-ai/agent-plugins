...
- `on` — trigger map. Supported keys match `.automation` triggers: `manual`, `webhook`, `schedule`, `file-change`, `notification`, `poll` (`{url, cron, mode?, headers?}`; `mode` is `"page"` (default) or `"feed"`; `headers` is a mapping `{HeaderName: "value"}` in YAML).
