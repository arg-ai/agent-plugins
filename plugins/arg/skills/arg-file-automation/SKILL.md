...
- `name` — display label.
- `uses` — an action kind (`read-file`, `write-file`, `copy-file`, `move-file`, `run-llm`, `run-automation` (see below), `http-request`, `send-notification`, …), `action/<action_id>` to run any action from the central registry (`with:` is then that action's own input, e.g. `uses: action/image_generate`), or a flow kind (`switch`, `merge`, `delay` (`with: {amount, unit}`), `filter` (`with: {arrayPath?, expression?}` — see Flow control above), or `loop` — see Loops below). **`uses: if` is a compile error** — conditionals are the step-level `if:` key below.
