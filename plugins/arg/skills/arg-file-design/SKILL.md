...
- Real DOM elements, static `::before` / `::after`, linear gradients, background images and inline SVG are supported. SVG computed fill, stroke and text styles are captured before detaching it from the page.
- Image fitting and position become native crop transforms. Polygon clipping and elliptical corners become paths. Complex gradients, photo color filters, solid border rings and shadow spread can become image paint within an editable layer; complex transformed SVG text can remain one SVG image. These fallbacks preserve appearance but reduce internal editability. Replacing a treated photo also replaces its baked effects.
- Text is measured in the browser, preserving observed line breaks and alignment. Each block descendant owns its own text, so it is not repeated inside the parent layer. Export wraps with the same resolved font used for glyph outlines. Font substitution can still change typography: embedded/custom font assets and custom variation axes are not persisted as native font resources. Native text holds one style per layer, so a `<strong>`, `<em>`, or coloured `<a>` inside a sentence becomes its own layer - keep inline styling for emphasis that earns a layer, and put a whole styled passage in its own block rather than mid-sentence when you want it editable as one thing.
...
- Relative `<img src>` and CSS `url()` paths resolve from the source file's folder. Workspace-absolute paths and data URLs also work. Interactive imports load resources through the open file's workspace; ordinary image fills retain durable workspace paths, while treated photo/background paints embed their image bytes. Do not put temporary signed asset URLs into template source.
...
### Visually accept the conversion

For a template library, HTML is a useful authoring route: compose the source, inspect its browser render, convert through the standard materializer, then inspect the native PNG at the same artboard dimensions. A structurally valid JSON document is not visual acceptance. Compare headline wording and line counts, image crops, outlines, gradients, clipping and overlap order. Keep text and geometry editable wherever possible and disclose any baked region.

The converter returns `htmlImport.report.notes` and structured `losses` with `element`, `property`, `fallback` (`image` or `omitted`) and `message`. The shared web/desktop conversion action shows notes on completion. Read these before treating a conversion as finished. Unsupported clip grammar, custom font axes and unavailable font resources require checking or repairing the affected region; scripts do not execute.

In the Arg repository, `pnpm --dir frontend test:design-html-import` compares source HTML, canonical JSON export and edited/reimported output. An ad hoc `--page` run requires `--max-mismatch` explicitly (a fraction, for example `0.01`). Inspect its source/native/diff PNGs and per-file reports as well as the score; a missing word can occupy less than one percent of a page.
...
**Fills:** Non-file paints are `solid` (`color`), `linear-gradient` (`angle`, `stops`, optional paired `start`/`end` points and `space` `local`/`world`; `angle` is the CSS angle - `0` runs bottom-to-top and `90` left-to-right, so the first stop sits at the bottom and the left respectively), `radial-gradient`/`angular-gradient`/`diamond-gradient` (`cx`,`cy`,`stops`), internal `shader` (`shaderId` plus optional `values`/`speed` - see below), `webcam` (source-less live camera), `qr` (a QR code - see below), and `none`. Every paint backed by a file uses one shape: `{ "type": "file", "fileType": "image"|"model3d"|"video"|"design"|"kml"|"cad"|"shader", "src": "...", "fileId"?: "..." }`; subtype settings stay alongside those common fields (`fit` for image/video/design, `model3d` pose, `kml` settings, `cad` settings, or `speed` for a shader file). Never author the old top-level `image`/`model3d`/`video`/`design`/`kml`/`cad` fill types or `shaderSrc`; the parser accepts and migrates them only for existing documents. Author a workspace path and preserve a valid existing `fileId`, but never invent one. A webcam stores only portable visual fields (`fit`, `mirrored`, `color`, `opacity`, `visible`) and never a device id, label, or stream identifier. A `qr` fill is `{ "type": "qr", "content": { "kind": "url", "url": "https://example.com" } }`: `content` is one of `{ "kind": "url", "url" }`, `{ "kind": "text", "text" }`, `{ "kind": "wifi", "ssid", "password"?, "security"?: "WPA"|"WEP"|"nopass", "hidden"? }`, `{ "kind": "email", "to", "subject"?, "body"? }`, `{ "kind": "phone", "number" }` or `{ "kind": "sms", "number", "message"? }`; optional `foreground` (default `#000000`), `background` (default `#ffffff`, or `"transparent"`), `errorCorrection` (`L`/`M`/`Q`/`H`, default `M`) and `margin` (quiet zone in modules, `0`-`16`, default `4`). The code is re-encoded from `content` on every render, so editing it updates the code everywhere; never store the encoded matrix or an image of it. Optional `fit` places the square code in the shape like an image fill's: `contain` (default - the whole code, centred), `cover` (cropped to fill the shape) or `fill` (stretched). Only `contain` is guaranteed to scan in a non-square shape, so prefer a square frame and leave `fit` unset. It works on artboards and exports as vector in SVG. The same fill draws any other barcode when you add `symbology`, a BWIPP id: `ean13`, `ean8`, `upca`, `upce`, `isbn`, `code128`, `gs1-128`, `code39`, `code93`, `interleaved2of5`, `itf14`, `rationalizedCodabar`, `datamatrix`, `pdf417`, `azteccode`, `maxicode`, `dotcode`, `hanxin`, `microqrcode`, `swissqrcode`, postal codes (`onecode`, `royalmail`, `auspost`, `postnet`) and the rest of BWIPP's list. Put the value to encode in `content` as `{ "kind": "text", "text": "5901234123457" }` - it must be valid for the symbology (EAN-13 takes 12 or 13 digits), or the fill renders empty. Optional `showText` (boolean) prints or hides the digits under a linear code; leave it unset for the symbology's own convention. `errorCorrection` applies to QR only. A linear barcode is wide and short, so give the shape roughly its proportions (an EAN-13 about 1.4:1) and keep `fit` at `contain`.
**Image fill `src`:** any workspace path the renderer can display — raster images (`.png`, `.jpg`, `.webp`, `.gif`, `.avif`), `.svg` (rendered natively), `.psd` (live reference: the renderer composites the Photoshop document to a flattened raster at display/export time; edits to the source PSD reflow on reload). On export, `.psd` fills bake to an inline PNG so the output is self-contained. Reference a real workspace path or a data URL; don't invent a path. A workspace path must be workspace-absolute (`/brand/logo.png`); a relative one is not resolved and renders as a broken fill. An image a user attached to an Arg chat is saved into the workspace automatically under `.chat-attachments/<chat>/<message>/...`; the attachment's note in the message carries that path, so use it with a leading slash rather than trying to re-encode the picture.
...
- There is no `image` object type - place a photo by giving a shape (usually a `rect`) a file fill (`{ "type": "file", "fileType": "image", "src", "fit" }`, with `fit` `cover`/`contain`/`fill`/`tile`/`crop`). Valid `src` values: any workspace image path (`.png`, `.jpg`, `.webp`, `.gif`, `.avif`), `.svg` workspace paths, `.psd` workspace paths (live reference - composited to a raster at display/export time), or a data URL - workspace paths workspace-absolute, never relative. Don't invent a path.
