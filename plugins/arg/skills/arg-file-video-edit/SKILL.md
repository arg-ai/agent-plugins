...
- `kind` is `"video"`, `"audio"`, or `"zoom"`. **Video tracks** hold `video`/`image`/`gif`/`cast`/`text`/`solid`/`adjustment`/`stock`/`weather`/`shape`/`embed`/`object3d`/`shader`/`kml`/`hyperframes` clips; **audio tracks** hold `audio` clips; **zoom tracks** hold `zoom` clips (the camera track — usually one per project).
...
- `type` — `video` / `audio` / `image` / `gif` / `cast` / `text` / `solid` / `adjustment` / `stock` / `weather` / `shape` / `embed` / `object3d` / `shader` / `kml` / `hyperframes` / `zoom`.
